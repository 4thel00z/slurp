...
from slurp.domain.ports import ProducerProtocol, QueueSubmitterProtocol


@dataclass
class ScrapeUsecase:
    app_config: AppConfig = None
    producer: ProducerProtocol = None
    submitter: QueueSubmitterProtocol = None
    ...
    def __post_init__(self):
        ...
        if self.app_config.confluence:
            self.producer = ConfluenceProducer(self.app_config.confluence, self.app_config.generator)
    ...
    async def run(self, ):
        ...
        async with self.submitter:
            ...
            async for i, task in aenumerate(self.producer(), start=1):
                if not task:
                    print(f"Received empty task, breaking")
                    break
